...
def expect_error_marker_pred(pred: Callable[[str], bool], expected: str) -> Iterator[None]:
    ...
    err_file = tdir / 'error'
    env_key = 'BPT_WRITE_ERROR_MARKER'
    prev = os.environ.get(env_key)
    try:
        os.environ[env_key] = str(err_file)
        yield
        assert False, 'bpt subprocess did not raise CallProcessError'
    except subprocess.CalledProcessError:
        assert err_file.exists(), \
            f'No error marker file [{err_file}] was generated, but bpt exited with an error (Expected "{expected}")'
        marker = err_file.read_text(encoding='utf-8').strip()
        assert pred(marker), \
            f'bpt did not produce the expected error (Expected {expected}, got {marker})'
    finally:
        shutil.rmtree(tdir)
        if prev:
