...
void pkg_cache::import_sdist(const sdist& sd, if_exists ife_action) {
    neo_assertion_breadcrumbs("Importing sdist archive", sd.id().to_string());
    if (!_write_enabled) {
        dds_log(critical,
    ...
    auto sd_dest = _root / sd.id().to_string();
    ...
    dds_log(info, "Source distribution for '{}' successfully imported", sd.id().to_string());
}
    ...
    return dds::solve(
        ...
        [&](std::string_view name) -> std::vector<pkg_id> {
            auto mine = ranges::views::all(_sdists)  //
                | ranges::views::filter([&](const sdist& sd) { return sd.id().name.str == name; })
                | ranges::views::transform([](const sdist& sd) { return sd.id(); });
        ...
        [&](const pkg_id& /* pkg_id */) {
            ...
            //!     | std::views::filter(NEO_TL(_1.for_kind == dep_for_kind::lib))  //
            //!     | ranges::to_vector;
            return std::vector<dds::dependency>{};
        });
}
