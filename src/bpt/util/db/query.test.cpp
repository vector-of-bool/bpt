...
TEST_CASE("Database operations") {
    ...
    auto  tups    = bpt::db_query<std::string, int>(get_row);
    auto [str, v] = *tups.begin();
    CHECK(str == "quux");
    CHECK(v == 42);
    get_row.reset();

    auto [str2, v2] = bpt::db_single<std::string, int>(get_row).value();
    CHECK(str2 == "quux");
    CHECK(v2 == 42);
    get_row.reset();

    auto single = bpt::db_cell<std::string>(db->prepare("SELECT bar FROM foo LIMIT 1"_sql));
    REQUIRE(single);
    CHECK(*single == "quux");
}
