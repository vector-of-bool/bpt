...
auto path_vec = [](auto... args) { return std::vector<bpt::fs::path>{args...}; };

TEST_CASE("Parse Makefile deps") {
    auto deps = bpt::parse_mkfile_deps_str("foo.o: bar.c");
    CHECK(deps.output == "foo.o");
    CHECK(deps.inputs == path_vec("bar.c"));
    // Newline is okay
    deps = bpt::parse_mkfile_deps_str("foo.o: bar.c \\\n baz.c");
    CHECK(deps.output == "foo.o");
    CHECK(deps.inputs == path_vec("bar.c", "baz.c"));

    deps = bpt::parse_mkfile_deps_str(
...
TEST_CASE("Invalid deps") {
    ...
    auto deps = bpt::parse_mkfile_deps_str("foo.o : cat");
    CHECK(deps.output.empty());
    CHECK(deps.inputs.empty());
    deps = bpt::parse_mkfile_deps_str("foo.c");
    CHECK(deps.output.empty());
    CHECK(deps.inputs.empty());
}
...
Something else
    ...
    auto  res        = bpt::parse_msvc_output_for_deps(mscv_output, "Note: including file:");
    auto& deps       = res.deps_info;
    auto  new_output = res.cleaned_output;
    CHECK(new_output == "\nOther line\n    indented line\nSomething else\n");
    CHECK(deps.inputs
          == std::vector<bpt::fs::path>({
